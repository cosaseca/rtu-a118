...
static void serial_omap_stop_tx(struct uart_port *port)
{
	...
	if(2 == up->port.line) {
//		gpio_set_value(5, 0);
//		printk(KERN_INFO"serial_omap_stop_tx %d %d %d\n",
//				up->tx_in_progress,
//				up->tx_wait_end,
//				up->rs485.flags);
	}
...
static void serial_omap_start_tx(struct uart_port *port)
{
	...
	if(2 == up->port.line) {
//		gpio_set_value(5, 1);
//		printk(KERN_INFO"serial_omap_start_tx %d %d %d\n",
//				up->tx_in_progress,
//				up->tx_wait_end,
//				up->rs485.flags);
	}
...
static inline irqreturn_t serial_omap_irq(int irq, void *dev_id)
{
	...
	unsigned long flags;
	if(2 == up->port.line) {
//		printk(KERN_INFO"serial_omap_irq %d %d %d\n",
//				up->tx_in_progress,
//				up->tx_wait_end,
//				up->rs485.flags);
	}
...
static void serial_omap_set_mctrl(struct uart_port *port, unsigned int mctrl)
{
	...
	if(2 == up->port.line) {
//		printk(KERN_INFO"serial_omap_set_mctrl %d %d %d\n",
//				up->tx_in_progress,
//				up->tx_wait_end,
//				up->rs485.flags);
	}
...
static int serial_omap_poll_get_char(struct uart_port *port)
{
	...
	printk(KERN_INFO"serial_omap_poll_get_char %d\n", up->port.line);
...
serial_omap_ioctl(struct uart_port *port, unsigned int cmd, unsigned long arg)
{
	...
	case 0x1001:
		spin_lock_irqsave(&up->port.lock, flags);
		if(2 == up->port.line) {
			gpio_set_value(GPIO_TO_PIN(0, 5), arg > 0?1:0);
//			printk("0x1001 %d\n", arg);
		} else if(4 == up->port.line) {
			gpio_set_value(GPIO_TO_PIN(3, 10), arg > 0?1:0);
		}
		spin_unlock_irqrestore(&up->port.lock, flags);
		break;
...
static int serial_omap_probe(struct platform_device *pdev)
{
	...
	//
	if(2 == up->port.line) {
		uart_omap_port_set_rts_gpio(up->port.line, GPIO_TO_PIN(0, 5));
	} else if(4 == up->port.line) {
		uart_omap_port_set_rts_gpio(up->port.line, GPIO_TO_PIN(3, 10));
	}//modify by ygz
	return 0;
err:
