...
static uint8_t sum8(uint8_t *buffer, uint16_t buffer_length) {
	uint16_t sum = 0; /* sum */
	//unsigned int i; /* will index into CRC lookup */
...
static uint16_t sum16(uint8_t *buffer, uint16_t buffer_length) {
	uint16_t sum = 0; /* sum */
	//unsigned int i; /* will index into CRC lookup */
...
#else
	int rc = read(ctx->s, rsp, rsp_length);
//	printf("-->rc %d\n", rc);
//	perror("-->rc: ");
	return rc;
#endif
...
/* The check_crc8 function shall return the message length if the CRC is
 valid. Otherwise it shall return -1 and set errno to EMBADCRC. add for zigbee */
int _modbus_rtu_check_integrity_zigbee(modbus_t *ctx, uint8_t *msg,
		const int msg_length) {
...
#endif
//	printf("msg_length %d\n", msg_length);
	sum_calculated = sum8(msg + 3, msg_length - 4);
	sum_received = 0xFF;
	/* Check Sum of msg */
	if ((sum_calculated == sum_received) && (0x7E == *msg)/*add head check*/) {
		return msg_length;
	} else {
		if (ctx->debug) {
			fprintf(stderr, "ERROR CRC received %0X != CRC calculated %0X\n",
					sum_calculated, sum_received);
		}
		if (ctx->error_recovery & MODBUS_ERROR_RECOVERY_PROTOCOL) {
			_modbus_rtu_flush(ctx);
		}
...
/* The check_crc8 function shall return the message length if the CRC is
 valid. Otherwise it shall return -1 and set errno to EMBADCRC. add for zigbee */
int _modbus_rtu_check_integrity_actuator(modbus_t *ctx, uint8_t *msg,
		const int msg_length) {
	uint16_t sum_calculated;
	uint16_t sum_received;
//	printf("msg_length %d\n", msg_length);
	sum_calculated = sum16(msg, msg_length - 4);
	sum_received = (msg[msg_length - 4] << 8) + msg[msg_length - 3];
	/* Check Sum of msg */
	if ((sum_calculated == sum_received) && (0xCC == *msg)/*add head check*/) {
		return msg_length;
	} else {
		if (ctx->debug) {
			fprintf(stderr, "ERROR CRC received %0X != CRC calculated %0X\n",
...
#if 1
	if(0 < ctx->msgfd) {
		FD_SET(ctx->msgfd, rfds);
	}
	int max_fd = ctx->s > ctx->msgfd?ctx->s:ctx->msgfd;
#endif
#if defined(_WIN32)
...
#else
//	printf("before select ------ max_fd %d ctx->msgfd %d\n", max_fd, ctx->msgfd);
	while ((s_rc = select(max_fd + 1, rfds, NULL, NULL, tv)) == -1) {
...
//	printf("before select ------ \n");
	if(ctx->msgfd > 0) {
		if(FD_ISSET(ctx->msgfd, rfds)) {
//			printf("before select ------ \n");
	//		MsgData *msg = ctx->msgbuf;
	//		printf("FD_ISSET self->zigbee_msg_fd %d\n", ctx->msgfd);
			pmsg_q_receive(ctx->msgfd, ctx->msgbuf, 0, NULL);
	//		printf("cmd: %d, len: %d\n", msg->cmd, msg->len, ctx->msgfun);
			if(NULL != ctx->msgfun) {
//				printf("before select ------ \n");
				ctx->msgfun(ctx->msgfd, (MsgData *)ctx->msgbuf);
			}
		}
	}
//	printf("before select ------ \n");
...
MODBUS_RTU_MAX_ADU_LENGTH, _modbus_set_slave, _modbus_rtu_build_request_basis,
		...
		_modbus_rtu_send_msg_pre, _modbus_rtu_send, _modbus_rtu_recv,
		_modbus_rtu_check_integrity,
		NULL, _modbus_rtu_connect, _modbus_rtu_close, _modbus_rtu_flush,
...
//add zigbee protocol analysis
const modbus_backend_t _modbus_rtu_zigbee_backend = { _MODBUS_BACKEND_TYPE_RTU,
_MODBUS_RTU_HEADER_LENGTH,
		_MODBUS_RTU_ZIGBEE_CHECKSUM_LENGTH, //_MODBUS_RTU_CHECKSUM_LENGTH,
		...
		_modbus_rtu_send, _modbus_rtu_recv,
		_modbus_rtu_check_integrity_zigbee, //_modbus_rtu_check_integrity,
		NULL, _modbus_rtu_connect, _modbus_rtu_close, _modbus_rtu_flush,
		_modbus_rtu_select, _modbus_rtu_filter_request };

modbus_t* modbus_new_rtu_zigbee(const char *device, int baud, char parity,
		int data_bit, int stop_bit) {
	modbus_t *ctx;
	modbus_rtu_t *ctx_rtu;
	size_t dest_size;
	...
	ctx->backend = &_modbus_rtu_zigbee_backend;
	ctx->backend_data = (modbus_rtu_t *) malloc(sizeof(modbus_rtu_t));
	ctx_rtu = (modbus_rtu_t *) ctx->backend_data;
