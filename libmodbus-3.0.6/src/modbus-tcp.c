...
ssize_t _modbus_udp_send(modbus_t *ctx, const uint8_t *req, int req_length)
{
	...
	return sendto(ctx->s, (const char*)req, req_length, MSG_NOSIGNAL
			, (struct sockaddr *)&ctx->ip_addr, ctx->addr_len);
//    return send(ctx->s, (const char*)req, req_length, MSG_NOSIGNAL);
}
...
static int _modbus_tcp_pi_connect(modbus_t *ctx)
{
    ...
    rc = getaddrinfo(ctx_tcp_pi->node, ctx_tcp_pi->service,
                     &ai_hints, &ai_list);
    //printf("rc: %d, node: %s, service: %s\n", rc, ctx_tcp_pi->node, ctx_tcp_pi->service);
    //perror("getaddrinfo");
    if (rc != 0)
        return rc;

    //printf("get addr info %d\n", ctx->s);
...
int modbus_tcp_pi_listen(modbus_t *ctx, int nb_connection)
{
    ...
    int new_socket;
    modbus_tcp_pi_t *ctx_tcp_pi = ctx->backend_data;
//    printf("%s %s %d\n", ctx_tcp_pi->node, ctx_tcp_pi->service, nb_connection);
...
int modbus_tcp_pi_udp_listen(modbus_t *ctx, int nb_connection)
{
	int rc;
	struct addrinfo *ai_list;
	struct addrinfo *ai_ptr;
