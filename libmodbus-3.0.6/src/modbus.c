...
#define MAX_A118_MESSAGE_LENGTH (260 + 1020 * 2)
...
static int send_msg(modbus_t *ctx, uint8_t *msg, int msg_length) {
	...
	if (ctx->debug) {
		for (i = 0; i < msg_length; i++)
			printf("[%.2X]", msg[i]); printf("\n");
	}

	/* In recovery mode, the write command will be issued until to be
	 successful! Disabled by default. */
#if 0//HAVE_DECL_TIOCSRS485
	if (ctx->backend->backend_type == _MODBUS_BACKEND_TYPE_RTU) {
		int serial_mode;
		modbus_rtu_t *ctx_rtu = ctx->backend_data;
		if(MODBUS_RTU_RS485 & ctx_rtu->serial_mode) {
			ioctl(ctx->s, 0x1001, 1);
		}
	}
#endif
	do {
...
/* Computes the length to read after the function received add for zigbee */
static uint8_t compute_meta_length_after_function_zigbee(int function,
		msg_type_t msg_type) {
	int length = 2;
#if 0
...
/* Computes the length to read after the meta information (address, count, etc) add for zigbee */
static int compute_data_length_after_meta_zigbee(modbus_t *ctx, uint8_t *msg,
		msg_type_t msg_type) {
#if 0
	int function = msg[ctx->backend->header_length];
#endif
...
/* Computes the length to read after the meta information (address, count, etc) add for zigbee */
static int compute_data_length_after_meta_actuator(modbus_t *ctx, uint8_t *msg,
		msg_type_t msg_type) {
	int length = 0;
	length = ((msg[ctx->backend->header_length + 2]) << 8)
				+ msg[ctx->backend->header_length + 3];
	length += 4;
	return length;
}
...
/* Waits a response from a modbus server or a request from a modbus client.
 ...
 - ETIMEDOUT
 - read() or recv() error codes
 - add this function for zigbee
 */

int receive_msg_zigbee(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type) {
	int rc;
	fd_set rfds;
	struct timeval tv;
	struct timeval *p_tv;
	 ...
	 * to reach the function code because all packets contain this
	 * information. */
	step = _STEP_META; //diff to rtu
	length_to_read = ctx->backend->header_length + 2; //diff to rtu
//	printf("length %d\n", length_to_read);
...
//		printf("rc = %d\n",rc);
		...
		if (length_to_read == 0) {
			...
			case _STEP_META:
				length_to_read = compute_data_length_after_meta_zigbee(ctx, msg,
						msg_type);
				if ((msg_length + length_to_read)
						> ctx->backend->max_adu_length) {
					errno = EMBBADDATA;
...
/* Waits a response from a modbus server or a request from a modbus client.
 ...
 - add this function for zigbee
 */
extern int _modbus_rtu_check_integrity_actuator(modbus_t *ctx, uint8_t *msg,
		const int msg_length);

int receive_msg_actuator(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type) {
	int rc;
	fd_set rfds;
	struct timeval tv;
	struct timeval *p_tv;
	 ...
	 * information. */
	step = _STEP_META; //diff to rtu
	length_to_read = ctx->backend->header_length + 4; //diff to rtu and zigbee
//	printf("length %d\n", length_to_read);
...
//		printf("rc = %d\n",rc);
		...
		if (length_to_read == 0) {
			...
			case _STEP_META:
				length_to_read = compute_data_length_after_meta_actuator(ctx, msg,
						msg_type);
				if ((msg_length + length_to_read)
						> ctx->backend->max_adu_length) {
					errno = EMBBADDATA;
...
//		printf("msg_length %d\n", msg_length);
	...
	return _modbus_rtu_check_integrity_actuator(ctx, msg, msg_length);
}
...
/* Receive the request from a modbus master */
int modbus_receive_zigbee(modbus_t *ctx, uint8_t *req) {
	return receive_msg_zigbee(ctx, req, MSG_INDICATION);
}

/* Receive the request from a modbus master */
int modbus_receive_zigbee_timeout(modbus_t *ctx, uint8_t *req, struct timeval *timeout) {
	return receive_msg_zigbee(ctx, req, MSG_TIMEOUT);
}

int modbus_receive_actuator_timeout(modbus_t *ctx, uint8_t *req, struct timeval *timeout) {
	return receive_msg_actuator(ctx, req, MSG_TIMEOUT);
}
...
/* Send a response to the received request.
 ...
 accordingly.
 */
int modbus_reply_old(modbus_t *ctx, const uint8_t *req, int req_length,
		modbus_mapping_t *mb_mapping) {
	int offset = ctx->backend->header_length;
	int slave = req[offset - 1];
	int function = req[offset];
//	uint16_t address = (req[offset + 1] << 8) + req[offset + 2];
	unsigned address = (req[offset + 1] << 8) + req[offset + 2];
//	printf("slave %d %d\n", slave, address);
	if(0 >= slave) {
		return 0;
	} else if(slave >= RTU_MAX_WELL_NUM) {
		address += (RTU_MAX_WELL_NUM - 1) * RTU_MAX_READ_REGISTER;//
	} else {
		address += (slave - 1) * RTU_MAX_READ_REGISTER;//
	}
//	address -= 1;
//	printf("address %d\n", address % RTU_MAX_READ_REGISTER + 1);
	...
	case _FC_READ_DISCRETE_INPUTS: {
		...
		break;
	case _FC_READ_HOLDING_REGISTERS: /*read lock*/{
		int nb = (req[offset + 3] << 8) + req[offset + 4];
		lock_modbus_rtu_lock(address, nb, function);// add lock
		if (nb < 1 || MODBUS_MAX_READ_REGISTERS < nb) {
			...
			rsp[rsp_length++] = nb << 1;
			for (i = address; i < address + nb; i++) {
				if(i % RTU_MAX_READ_REGISTER < 300) {
					flush_time = 1;
					rsp[rsp_length++] = mb_mapping->tab_registers[i % RTU_MAX_READ_REGISTER] >> 8;
					rsp[rsp_length++] = mb_mapping->tab_registers[i % RTU_MAX_READ_REGISTER] & 0xFF;
				} else {
...
//				struct tm* st_time;
//				st_time = localtime(&a);
//				mb_mapping->tab_registers[4] = int16_to_bcd(st_time->tm_year + 1900);
//				mb_mapping->tab_registers[5] = int16_to_bcd(st_time->tm_mon + 1);
//				mb_mapping->tab_registers[6] = int16_to_bcd(st_time->tm_mday);
//				mb_mapping->tab_registers[7] = int16_to_bcd(st_time->tm_hour);
//				mb_mapping->tab_registers[8] = int16_to_bcd(st_time->tm_min);
//				mb_mapping->tab_registers[9] = int16_to_bcd(st_time->tm_sec);
			}
		}
		lock_modbus_rtu_unlock(address, nb, function);//
	}
		break;
	case _FC_READ_INPUT_REGISTERS: {
	...
	case _FC_WRITE_SINGLE_COIL:
		...
		break;
	case _FC_WRITE_SINGLE_REGISTER:
		lock_modbus_rtu_lock(address, 1, function);// add lock
		if (address >= mb_mapping->nb_registers) {
			...
			if(address % RTU_MAX_READ_REGISTER < 300) {
				mb_mapping->tab_registers[address % RTU_MAX_READ_REGISTER] = data;
//				printf("%d, data %d\n", address % RTU_MAX_READ_REGISTER, data);
				if(address % RTU_MAX_READ_REGISTER == 29 && data == 1) {
//					printf("data %d\n", data);
...
//					int rc = MSG_CONFIG_START;
					...
					mb_mapping->tab_registers[address % RTU_MAX_READ_REGISTER] = 0;
				}
			} else {
				int flash_save_switch = 1;
				switch(address % RTU_MAX_READ_REGISTER + 1) {
				case 329://油压采集间隔
					break;
				case 336://套压采集间隔
					break;
...
//				printf("slave %d %d %d %d %d\n", flash_save_switch, slave,
//						RTU_MAX_WELL_NUM,
//						((1 == flash_save_switch) && (slave < RTU_MAX_WELL_NUM))
//						,address % RTU_MAX_READ_REGISTER + 1);
				if((1 == flash_save_switch) && (slave < RTU_MAX_WELL_NUM)) {
					int rc[2] = {MSG_CONFIG2_START, slave};
					pmsg_q_send(rtu_a118_t.zigbee_msg_fd, rc, sizeof(rc), 0);
				}
			}
			memcpy(rsp, req, req_length);
			rsp_length = req_length;
		}
		lock_modbus_rtu_unlock(address, 1, function);//
		break;
	case _FC_WRITE_MULTIPLE_COILS: {
		int nb = (req[offset + 3] << 8) + req[offset + 4];
		...
		break;
	case _FC_WRITE_MULTIPLE_REGISTERS: /*write lock*/{
		int nb = (req[offset + 3] << 8) + req[offset + 4];
		lock_modbus_rtu_lock(address, nb, function);// add lock
		if (nb < 1 || MODBUS_MAX_WRITE_REGISTERS < nb) {
			...
			int i, j;
			int flash_save_switch = 0;
			for (i = address, j = 6; i < address + nb; i++, j += 2) {
				/* 6 and 7 = first value */
				if(i % RTU_MAX_READ_REGISTER < 300) {
					mb_mapping->tab_registers[i % RTU_MAX_READ_REGISTER] = (req[offset + j] << 8)
							+ req[offset + j + 1];
					if(address % RTU_MAX_READ_REGISTER == 0x30 && mb_mapping->tab_registers[i % RTU_MAX_READ_REGISTER] == 1) {
						int rc = MSG_CONFIG_START;
						pmsg_q_send(rtu_a118_t.zigbee_msg_fd, &rc, sizeof(int), 0);
						pmsg_q_send(rtu_a118_t.tcp_server_msg_fd, &rc, sizeof(int), 0);
						mb_mapping->tab_registers[i % RTU_MAX_READ_REGISTER] = 0;
					}
				} else {
					switch(address % RTU_MAX_READ_REGISTER + 1) {
					case 329://油压采集间隔
	...
	case _FC_WRITE_AND_READ_REGISTERS: /*write read lock*/{
		int nb = (req[offset + 3] << 8) + req[offset + 4];
		uint16_t address_write = (req[offset + 5] << 8) + req[offset + 6];
		int nb_write = (req[offset + 7] << 8) + req[offset + 8];
		int nb_write_bytes = req[offset + 9];

		if (nb_write < 1 || MODBUS_MAX_RW_WRITE_REGISTERS < nb_write || nb < 1
				|| MODBUS_MAX_READ_REGISTERS < nb
			...
			lock_modbus_rtu_lock(address_write, nb_write, 1);// add lock
			...
			lock_modbus_rtu_unlock(address_write, nb_write, 1);// add lock
...
int modbus_reply(modbus_t *ctx, const uint8_t *req, int req_length,
	...
	int function = req[offset];
	uint16_t address = (req[offset + 1] << 8) + req[offset + 2];
	uint8_t rsp[MAX_A118_MESSAGE_LENGTH]; //default MAX_MESSAGE_LENGTH
	int rsp_length = 0;
	sft_t sft;
	if(slave < 1 || slave > MAX_WELL_NUM) {
		return 0;
	}
	...
	case _FC_READ_HOLDING_REGISTERS: {
		...
		if (nb < 1 || MODBUS_MAX_READ_REGISTERS < nb) {
			...
			rsp_length = response_exception(ctx, &sft,
					MODBUS_EXCEPTION_ILLEGAL_DATA_VALUE, rsp);
		} else if ((address + nb) > RTU_MAX_READ_REGISTER) {
			if (ctx->debug) {
				fprintf(stderr, "Illegal data address %0X in read_registers\n",
						address + nb);
			}
		...
		else
		{
			rsp_length = ctx->backend->build_response_basis(&sft, rsp);
			rsp[rsp_length++] = nb << 1;
			map_modbus_read_registers((uint16_t *)(rsp + rsp_length), nb, slave, address, 1);
			rsp_length += 2 * nb;
		}
	}
		break;
	case _FC_WRITE_SINGLE_REGISTER:
		if (address >= RTU_MAX_READ_REGISTER) {
			if (ctx->debug) {
				fprintf(stderr, "Illegal data address %0X in write_register\n",
						address);
			}
			...
			map_modbus_write_registers((uint16_t *)(req + offset + 3), 1, slave, address, 1);
			memcpy(rsp, req, req_length);
			rsp_length = req_length;
		}
		break;
	case _FC_WRITE_MULTIPLE_REGISTERS: {
		...
		{
			map_modbus_write_registers((uint16_t *)(req + offset + 6), nb, slave, address, 1);
			/* 6 and 7 = first value */
			rsp_length = ctx->backend->build_response_basis(&sft, rsp);
			/* 4 to copy the address (2) and the no. of registers */
			memcpy(rsp + rsp_length, req + rsp_length, 4);
